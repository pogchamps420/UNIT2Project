...
void Menu::PrintMenu()
{
	...
	{
		std::cout << itemIterator->second.identifier << " - " << itemIterator->second.description << ": " << itemIterator->second.value << '\n';
		itemIterator++;
	}
	std::cout << "-----------------------\n";
...
{
	if (_options[identifier].visibility != availability)
	{
...
void Menu::ChangeItemValue(char identifier, std::string newValue)
{
...
{
	_items[identifier].ChangeValue(std::to_string(newValue));
}
